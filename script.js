...
displaySingleData = (singleData) => {
  ...
  cardText.innerHTML = `
<div class="text-primary card-text " >${singleData.pricing[0].price}<br>${singleData.pricing[0].plan}</div>
<div class="text-danger card-text ">${singleData.pricing[1].price}<br>${singleData.pricing[1].plan}</div>
<div class="text-success  card-text">${singleData.pricing[2].price}<br>${singleData.pricing[2].plan}</div>`;
...
<li>${
    singleData.integrations[0]
      ? singleData.integrations[0]
      : (integrations.innerHTML = "NO DATA FOUND")
  }</li>
<li>${
    singleData.integrations[1]
      ? singleData.integrations[1]
      : (integrations.innerHTML = "NO DATA FOUND")
  }</li>
<li>${
    singleData.integrations[2]
      ? singleData.integrations[2]
      : (integrations.innerHTML = "NO DATA FOUND")
  }</li>
...
`;
  ...
  imgCard.innerHTML = `
<img src="${
    singleData.image_link[0] ? singleData.image_link[0] : singleData.logo
...
`;
  ...
  const cardSpeechDescription = document.getElementById(
    "cardSpeechDescription"
  );
...
const ascendingCard = () => {
  fetch("https://openapi.programming-hero.com/api/ai/tools")
    .then((response) => response.json())
    .then((data) => displayAscendingData(data.data.tools));

  const displayAscendingData = (data) => {
    console.log(data);
    let dateArray = [];
    data.map((cardItem) => {
      return dateArray.push(cardItem.published_in);
    });
...
const displayData = (datas) => {
  ...
  datas.forEach((data) => {
    ...
    div.innerHTML = `
        ...
        <div class=" container mt-3 ">  <img   class="img-fluid rounded-4"src="${
          data.image
        }" alt="" srcset=""></div>
        ...
        <div class="d-flex justify-content-between align-items-center container">
          ...
          <section onclick="openModal(${parseFloat(
            data.id
          )})" class="rounded-5 btn btn-danger "   data-bs-toggle="modal" data-bs-target="#exampleModal">
         ...
         <div class="col-sm-6">
            <div class="card " >
               ...
               <div style="position:relative;">
                      ...
                      <div id="cardSpeechTitle" class="my-5"> </div>
